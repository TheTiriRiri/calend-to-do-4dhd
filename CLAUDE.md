...
Playwright: chromium always runs; webkit project only registers when `E2E_WEBKIT` is set (see above). Viewport 390x844, `reuseExistingServer` outside CI. Shared e2e helpers live in [app/tests/e2e/utils.ts](app/tests/e2e/utils.ts) (`bypassOnboarding`, `addTaskScheduled`, `localDay`) - put a helper there rather than copying it into a second spec, and keep the file free of `test()` blocks so Playwright's `*.spec.ts` match ignores it. Most specs bypass onboarding with `test.beforeEach(({ page }) => bypassOnboarding(page))`; `onboarding.spec.ts` is the one flow that runs it for real. Modal buttons must be scoped (e.g. `page.locator('.sheet')`, or `.overlay .sheet` on screens like the calendar that have other `.sheet` elements in the DOM) - the underlying screen stays in the DOM behind a sheet and shares button labels like "Dodaj".
